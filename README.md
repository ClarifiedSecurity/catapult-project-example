...
The project has an example host called `monolith` that can be used to set up Vault/Keycloak/Providentia/Nexus/Samba on a pre-existing VM. Just fill out the required variables in `host_vars/monolith.yml` and run `ctp host deploy monolith`.
